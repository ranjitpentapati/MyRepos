...
public class GetUserAccountCommand extends HystrixCommand<UserAccount> {
	...
	protected UserAccount run() {
		...
		return new UserAccount(86975, "John James", 2);
	}

	/**
	 * Use the HttpCookie value as the cacheKey so multiple executions in the same HystrixRequestContext will respond
	 * from cache.
	 */
	@Override
	...
	/**
	 * Fallback that will use data from the UserCookie and stubbed defaults to create a UserAccount if the network call
	 * failed.
	 */
	@Override
	protected UserAccount getFallback() {
		...
		return new UserAccount(userCookie.userId, userCookie.name, userCookie.accountType);
	}
