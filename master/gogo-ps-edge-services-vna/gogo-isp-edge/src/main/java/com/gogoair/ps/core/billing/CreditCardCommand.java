package com.gogoair.ps.core.billing;
...
/**
 * RB: This code was taken from Netflix OSS sample - its a good example for us - will totally change as we move forward but ironically its totally in the area that I needed i.e. credit card payments.
 * 
 * class was originally taken from a functional example using the Authorize.net API
 ...
 * and it would behave idempotently so that it would not result in duplicate transactions and each would return a
 * successful response as if it was the first-and-only execution.
 * <p>
 * This idempotence (within the duplicate window time frame set to multiple hours) allows for clients that experience
 * timeouts and failures to confidently retry the credit card transaction without fear of duplicate credit card charges.
 * <p>
 * This in turn allows the HystrixCommand to be configured for reasonable timeouts and isolation rather than letting it
 * go 10+ seconds hoping for success when latency occurs.
 * <p>
 * In this example, the timeout is set to 3,000ms as normal behavior typically saw a credit card transaction taking
 * around 1300ms and in this case it's better to wait longer and try to succeed as the result is a user error.
 * <p>
...
public class CreditCardCommand extends HystrixCommand<CreditCardAuthorizationResult> {
	...
	/**
	 * A HystrixCommand implementation accepts arguments into the constructor which are then accessible to the
	 * <code>run()</code> method when it executes.
	 * 
	 * @param order
	 * @param payment
	 * @param amount
	...
	private CreditCardCommand(AuthorizeNetGateway gateway, Order order, PaymentInformation payment, BigDecimal amount) {
		super(Setter.withGroupKey(HystrixCommandGroupKey.Factory.asKey("CreditCard"))
				// defaulting to a fairly long timeout value because failing a credit card transaction is a bad user
				// experience and 'costly' to re-attempt
	...
	/**
	 * Actual work of submitting the credit card authorization occurs within this <code>HystrixCommand.run()</code>
	 * method.
	 */
	@Override
	protected CreditCardAuthorizationResult run() {
		...
		if (result.isApproved()) {
			return CreditCardAuthorizationResult.createSuccessResponse(result.getTarget().getTransactionId(), result.getTarget()
			...
			if (result.getReasonResponseCode().getResponseReasonCode() == 11) {
				if (result.getTarget().getAuthorizationCode() != null) {
					...
					// This is part of the idempotent behavior we require so that we can safely timeout and/or fail and
					// allow
					// client applications to re-attempt submitting a credit card transaction for the same order again.
					// In those cases if the client saw a failure but the transaction actually succeeded, this will
					// capture the
					// duplicate response and behave to the client as a success.
					return CreditCardAuthorizationResult.createDuplicateSuccessResponse(result.getTarget().getTransactionId(), result
			...
			return CreditCardAuthorizationResult.createFailedResponse(result.getReasonResponseCode() + " : " + result.getResponseText());
			/**
			 * NOTE that in this use case we do not throw an exception for an "error" as this type of error from the
			 * service is not a system error, but a legitimate usage problem successfully delivered back from the
			 * service.
			 * 
			 * Unexpected errors will be allowed to throw RuntimeExceptions.
			 * 
			 * The HystrixBadRequestException could potentially be used here, but with such a complex set of errors and
			 * reason codes it was chosen to stick with the response object approach rather than using an exception.
			 */
		}
	}
	 ...
	 * 
	 * They are statically mocked in this example to demonstrate how Hystrix might behave when wrapping this type of
	 * call.
	 */
